-- Fix RLS policies for roof leads functionality (Column Reference Fix)
-- Addresses "column 'created_by' does not exist" error by using correct column names
...
DROP POLICY IF EXISTS "prospects_tenant_access" ON public.prospects;
DROP POLICY IF EXISTS "accounts_tenant_access" ON public.accounts;
DROP POLICY IF EXISTS "properties_tenant_access" ON public.properties;

-- Drop problematic policies from previous migration
DROP POLICY IF EXISTS "roof_leads_user_access" ON public.roof_leads;
DROP POLICY IF EXISTS "roof_leads_manager_tenant_access" ON public.roof_leads;
DROP POLICY IF EXISTS "roof_lead_images_user_access" ON public.roof_lead_images;
DROP POLICY IF EXISTS "roof_lead_images_manager_access" ON public.roof_lead_images;
DROP POLICY IF EXISTS "user_profiles_full_access" ON public.user_profiles;
DROP POLICY IF EXISTS "manager_team_profiles_access" ON public.user_profiles;
...
-- Create roof_leads RLS policies using correct column name (created_by exists in roof_leads)
...
-- Create roof_lead_images RLS policies using correct column name (uploaded_by exists in roof_lead_images)
...
-- For prospects, accounts, and properties tables - use tenant-only access without created_by reference
-- This avoids the "column created_by does not exist" error
CREATE POLICY "prospects_tenant_access_fixed"
ON public.prospects
FOR ALL
TO authenticated
USING (
...
-- Accounts table RLS - tenant-scoped access without created_by reference
CREATE POLICY "accounts_tenant_access_fixed"
ON public.accounts
FOR ALL
TO authenticated
USING (
...
-- Properties table RLS - tenant-scoped access without created_by reference
CREATE POLICY "properties_tenant_access_fixed"
